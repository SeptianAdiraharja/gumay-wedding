...
<div class="mb-8">
    ...
    <h1 class="font-display text-3xl text-ivory">Edit Rekomendasi Makeup</h1>
</div>
...
<form action="{{ route('admin.recommendations.update', $recommendation) }}" method="POST"
        ...
        <div>
            ...
            <select name="skin_type_id" required
                ...
                @foreach($skinTypes as $st)
                    <option value="{{ $st->id }}" {{ old('skin_type_id', $recommendation->skin_type_id) == $st->id ? 'selected' : '' }}>
                        {{ $st->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="category" required
                ...
                @foreach(['primer','foundation','concealer','bedak','blush','eye','lip','setting_spray'] as $cat)
                    <option value="{{ $cat }}" {{ old('category', $recommendation->category) === $cat ? 'selected' : '' }}>
    ...
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $recommendation->title) }}" required
    ...
    <div>
        ...
        <textarea name="description" rows="4"
                  class="w-full bg-surface border border-gold/20 focus:border-gold focus:ring-1 focus:ring-gold text-ivory rounded-xl text-sm p-4 outline-none transition duration-150 resize-none placeholder-ink/30">{{ old('description', $recommendation->description) }}</textarea>
    </div>
