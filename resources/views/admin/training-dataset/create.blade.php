...
@section('title', 'Tambah Data Latih — Naive Bayes')
...
<div class="mb-8">
    ...
    <h1 class="font-display text-3xl text-ivory">Tambah Data Latih</h1>
</div>

<!-- Card Form Melebar -->
<form action="{{ route('admin.training-dataset.store') }}" method="POST"
    ...
    <div>
        ...
        <select name="skin_type_id" required
            ...
            <option value="" disabled {{ old('skin_type_id') ? '' : 'selected' }} class="bg-plum text-ink/50">-- Pilih Jenis Kulit --</option>
            @foreach($skinTypes as $st)
                <option value="{{ $st->id }}" {{ old('skin_type_id') == $st->id ? 'selected' : '' }} class="bg-plum text-ivory">
                    {{ $st->name }}
                </option>
            @endforeach
        </select>
    ...
    <!-- Parameter Fitur (Grid 3 Kolom pada Layar Sedang/Besar) -->
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <select name="tingkat_minyak" required
                    ...
                    @foreach(['rendah','sedang','tinggi'] as $opt)
                        <option value="{{ $opt }}" {{ old('tingkat_minyak') === $opt ? 'selected' : '' }} class="bg-plum text-ivory">
                            {{ ucfirst($opt) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="tingkat_kering" required
                        class="w-full bg-surface border border-gold/20 focus:border-gold focus:ring-1 focus:ring-gold text-ivory rounded-xl text-sm px-4 py-3 outline-none transition duration-150 cursor-pointer">
                    @foreach(['rendah','sedang','tinggi'] as $opt)
                        <option value="{{ $opt }}" {{ old('tingkat_kering') === $opt ? 'selected' : '' }} class="bg-plum text-ivory">
                            {{ ucfirst($opt) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="pori_pori" required
                    ...
                    @foreach(['kecil','sedang','besar'] as $opt)
                        <option value="{{ $opt }}" {{ old('pori_pori') === $opt ? 'selected' : '' }} class="bg-plum text-ivory">
                            {{ ucfirst($opt) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="penggunaan_skincare" required
                    ...
                    @foreach(['ya' => 'Rutin', 'tidak' => 'Tidak Rutin'] as $opt => $label)
                        <option value="{{ $opt }}" {{ old('penggunaan_skincare') === $opt ? 'selected' : '' }} class="bg-plum text-ivory">
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="jerawat" required
                    ...
                    @foreach(['ya','tidak'] as $opt)
                        <option value="{{ $opt }}" {{ old('jerawat') === $opt ? 'selected' : '' }} class="bg-plum text-ivory">
                            {{ ucfirst($opt) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="sensitivitas" required
                        class="w-full bg-surface border border-gold/20 focus:border-gold focus:ring-1 focus:ring-gold text-ivory rounded-xl text-sm px-4 py-3 outline-none transition duration-150 cursor-pointer">
                    @foreach(['rendah','sedang','tinggi'] as $opt)
                        <option value="{{ $opt }}" {{ old('sensitivitas') === $opt ? 'selected' : '' }} class="bg-plum text-ivory">
                            {{ ucfirst($opt) }}
                        </option>
                    @endforeach
                </select>
